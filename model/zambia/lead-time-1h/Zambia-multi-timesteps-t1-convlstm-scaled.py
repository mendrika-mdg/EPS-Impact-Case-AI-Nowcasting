...
# === ConvLSTM Cell (unchanged interface) ===
...
# === Scaled MultiStep Model ===
class MultiStepTabularConvLSTM(nn.Module):
    def __init__(self, input_size_per_step=17, lstm_hidden=[512, 256], projection_size=8, seq_len=3):
        super().__init__()
        ...
        self.fc = nn.Linear(lstm_hidden[1], projection_size * projection_size * 64)
        self.relu = nn.ReLU()

        self.convlstm = ConvLSTMCell(input_dim=64, hidden_dim=64, kernel_size=3, padding=1)
    ...
    def forward(self, x_seq):
        ...
        for t in range(self.seq_len):
            ...
            x_t = x_t.view(-1, 64, self.projection_size, self.projection_size)
...
class LightningMultiStepConvLSTM(pl.LightningModule):
    def __init__(self, lr=1e-3, threshold=0.15):
        super().__init__()
