...
class IterableShardDataset(IterableDataset):
    def __init__(self, shard_dir: str, split_by_rank: bool = True):
        ...
        all_files = sorted(f for f in os.listdir(shard_dir) if f.endswith("_proc.pt"))[:-1]
        if split_by_rank:
...
def main():
    ...
    logger = WandbLogger(project="pancast-self", name="pancast-10km-self-bilinear", log_model=True) if LOCAL_RANK == 0 else None
