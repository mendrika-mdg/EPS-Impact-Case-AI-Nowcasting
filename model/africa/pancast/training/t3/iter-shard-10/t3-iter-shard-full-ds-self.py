...
def compute_fss(preds, targets, window: int = 4):
    ...
    return 1.0 - (mse / (ref + 1e-6))


class SelfFSSLoss(nn.Module):
    def __init__(self, window_size: int = 4, pos_weight: Optional[float] = None, alpha: float = 0.3):
        super().__init__()
    ...
    def __iter__(self) -> Iterator:
        ...
        for path in shard_paths:
            try:
                ...
                print(f"[Rank {LOCAL_RANK}] Failed to load {path}: {e}")
                continue
...
def main():
    ...
    logger = WandbLogger(project="pancast-t3", name="pancast-10km-self-bilinear", log_model=True) if LOCAL_RANK == 0 else None
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ModelCheckpoint(monitor="val_auc", mode="max", filename="best-core2map", save_top_k=1, verbose=True),
            EarlyStopping(monitor="val_auc", mode="max", patience=5, min_delta=0.001, verbose=True)
        ]
    )
